...
// @ts-ignore
import { onPrint, calcRT, getPageText, savePDF } from "svelte-pdf/utils/Helper.svelte";
// @ts-ignore
import Tooltip from "svelte-pdf/utils/Tooltip.svelte";
...
const initialLoad = async () => {
    ...
    .then(async function (pdfDoc_) {
        ...
        for (let number = 1; number <= totalPages; number++) {
            ...
            // Show the text of the page in the console -> doesn't work?
            pdfContent = pdfContent.concat(textPage);
            readingTime = calcRT(pdfContent);
            });

             // Check for questions text
            getPageText(number, pdfDoc).then(function(result) {
...
const onPageTurn = () => {
    ...
    if (autoFlip === true && pageNum <= totalPages) {
    //countdown seconds
...
<div class={showBorder === true ? "control" : "null"}>
    ...
    <div class="control-start">
        ...
        <Tooltip>
            ...
            <span
            slot="activator"
            class="button-control {pageNum >= totalPages ? 'disabled' : null}"
            on:click={() => onNextPage()}
            on:keydown
            >
            <svg
